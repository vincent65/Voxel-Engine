...
def ray_casting(screen_array, player_pos, player_angle, player_height, player_pitch,
    ...
    #calculate the first ray angle
    ray_angle = player_angle - h_fov
    for num_ray in range(screen_width):
        #loop through each trig value for each ray angle
        first_contact = False
        sin_a = math.sin(ray_angle)
        cos_a = math.cos(ray_angle)
        #find the coordinates of the current point into the specified distance
        #filter out coordinates that do not fall within the texture map
        for depth in range(1, ray_distance):
            ...
            if 0 < x < map_width:
                ...
                if 0 < y < map_height:
                    ...
                    #draw a vertical line if the height is less than the vertical buffer on screen
                    if height_on_screen < y_buffer[num_ray]:
                        for screen_y in range(height_on_screen, y_buffer[num_ray]):
                            #assign color based on texture map
                            screen_array[num_ray, screen_y] = color_map[x, y]
                        #override heigth value in buffer
